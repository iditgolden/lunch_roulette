...
profile_directory = ".mozilla/firefox/tenbis/"
...
class TenBisBrowser(Browser):
    def __init__(self, downloaded_data_dir=None, implicitly_wait_seconds=10, is_visible=False, is_profile_dir=False):
        profile_directory = None
        if is_profile_dir:
            p_dir = get_profile_dir()
            if os.path.isdir(p_dir):
                profile_directory = p_dir
            else:
                log.warn("Path to firefox profile does not exist. running as default user.")
        super(TenBisBrowser, self).__init__(
            downloaded_data_dir=downloaded_data_dir, implicitly_wait_seconds=implicitly_wait_seconds,
            profile_directory=profile_directory, is_visible=is_visible)


def get_profile_dir():
    return os.path.join(os.path.expanduser("~"), profile_directory)
